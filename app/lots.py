...
@lots_bp.route('/produtos/novo', methods=['GET', 'POST'])
@login_required
def create():
    ...
    if request.method == 'POST':
        try:
            ...
            if not db.session.get(Loja, store) or not db.session.get(Setor, sector):
                raise ValueError()
            if len(reason) > 255 or len(plu) > 50:
                raise ValueError()
            item = Produto(nome_produto=product['nome'], barcode=product['barcode'], marca=product['marca'],
                source=product['source'], source_url=product['source_url'], plu=plu, quantidade=quantity,
                validade=validity, loja_id=store, setor_id=sector, motivo_rebaixa=reason,
